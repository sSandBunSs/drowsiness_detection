...
Pengembangan dan pengujian awal sistem dilakukan pada sebuah PC berbasis Windows 10 Pro dengan prosesor Intel Core i7-4790, RAM 10GB, GPU RX550 4GB, dan penyimpanan SSD 512GB, dilengkapi sebuah webcam USB sebagai sumber input kamera. Selain PC, penelitian ini juga merencanakan pengujian pada Raspberry Pi 4 untuk menilai kelayakan sistem pada perangkat embedded berbiaya rendah; akses ke hardware fisik tidak tersedia, sehingga performa RPi4 diestimasi melalui proksi core ARM Cortex-A72 (AWS EC2 `a1.medium`) — keterbatasan yang dinyatakan eksplisit, bukan pengujian yang belum selesai (metodologi lengkap di BAB V §5.2.3).
...
> 🔶 DRAF PLACEHOLDER — §5.2.1–§5.2.5 sudah diisi data asli. §5.2.3 (PC vs Raspberry Pi 4) menggunakan estimasi berbasis core Cortex-A72 (AWS EC2 `a1.medium`) karena akses hardware RPi4 fisik tidak tersedia — lihat metodologi lengkap di §5.2.3. **Jangan mengisi angka `[ISI]` yang tersisa dengan tebakan/asumsi.**
...
Grafik EAR/MAR/PERCLOS (sumbu-Y) terhadap waktu (sumbu-X) untuk kedua sesi §5.2.2, dengan interval `DROWSY` dari ground truth diarsir merah, dihasilkan via `plot_trends.py logs/metrics_<sesi>.csv ground_truth[_malam].csv <output.png>` — skrip ini memakai ulang `load_ground_truth` dari `validate_accuracy.py` agar interval yang diarsir identik dengan yang dipakai untuk skor akurasi. Disimpan sebagai **Gambar 5.1** (`figures/gambar_5_1_trend_siang.png`, sesi siang) dan **Gambar 5.2** (`figures/gambar_5_2_trend_malam.png`, sesi malam).
...
> Argumen di bawah sudah diisi dengan angka nyata dari §5.1, §5.2.1–§5.2.5, dan §5.3.
...
Eksperimen proksi (§5.3) menunjukkan bahwa personalisasi threshold hanya bermakna jika baseline berasal dari satu individu — konsisten dengan desain `_calibrate()` dan berbeda dari precedent Ersoy dkk. (2026) yang juga memakai formula 75%×baseline namun pada baseline per-individu asli (naik 1,53% dari fixed), bukan baseline populasi campuran seperti proksi pada penelitian ini. Perbandingan berpasangan pada baseline individu asli (§5.2.5) mengonfirmasi arah yang sama: threshold adaptif (0,301) mencapai 62,59% vs fixed (0,25) 60,39% pada sesi siang — selisih +2,20 poin persentase, searah dengan (meski lebih besar magnitudenya dari) kenaikan 1,53% Ersoy dkk., dengan kontribusi terbesar pada recall DROWSY (0,256 vs 0,077).
...
> Status: kesimpulan #1–#3 sudah terisi angka nyata, termasuk perbandingan berpasangan adaptive-vs-fixed pada baseline individu yang sama (BAB V §5.2.5), siang/malam (§5.2.2), dan estimasi RPi4 (§5.2.3, proksi Cortex-A72 — bukan pengujian hardware fisik).
...
Pengujian video real-time per-individu (baseline kalibrasi asli, bukan proksi) mencapai akurasi 62,59% pada sesi siang — jauh di bawah 95,1% Zhu dkk. (2022, jurnal 3), dengan penyebab spesifik pada logika prioritas status (`DROWSY` mengalahkan `WARNING` tanpa syarat) dan ketiadaan hysteresis pada counter EAR, bukan pada konsep EAR/MAR/PERCLOS itu sendiri. Perbandingan siang/malam menemukan penurunan akurasi 7,29 poin persentase (62,59%→55,30%) — lebih kecil secara absolut dibanding drop pada literatur berthreshold tetap (jurnal 13, jurnal 6), mendukung argumen adaptive threshold menjembatani gap pencahayaan, meski dengan efek samping precision NORMAL turun (0,742→0,589) saat malam. Perbandingan berpasangan adaptive-vs-fixed pada baseline individu yang sama menunjukkan threshold adaptif (0,301) mencapai 62,59% vs fixed (0,25) 60,39% — selisih +2,20 poin persentase, searah dengan kenaikan 1,53% Ersoy dkk. (2026). Estimasi performa Raspberry Pi 4 (proksi core Cortex-A72, AWS EC2 `a1.medium`, karena akses hardware fisik tidak tersedia) mencapai 20,96 FPS/45,70 ms, konsisten dengan FPS 10 Moredo dkk. (2025, jurnal 8) setelah dikoreksi rasio clock speed — jauh di atas kebutuhan real-time minimum untuk mendeteksi kantuk berdurasi detik, meski reliabilitas 91,09% Alvarez Oviedo dkk. (2025, jurnal 7) adalah metrik akurasi berbeda dimensi, tidak dibandingkan head-to-head dengan FPS di sini.

### 5. Kesimpulan

Sistem deteksi kantuk berbasis EAR+MAR+PERCLOS dengan adaptive threshold berhasil diimplementasikan dan menunjukkan bahwa threshold tetap sensitif terhadap variasi data, mendukung kebutuhan personalisasi per-pengemudi. Validasi pada video real-time per-individu mengonfirmasi manfaat personalisasi secara langsung: threshold adaptif unggul +2,20 poin persentase dibanding fixed pada baseline individu yang sama.
...
| BAB V §5.2 | Lengkap untuk §5.2.1–§5.2.5 (data video real-time, siang/malam, estimasi performa PC vs RPi4 via proksi Cortex-A72, grafik tren, dan perbandingan berpasangan adaptive-vs-fixed) |
| BAB V §5.4 Pembahasan | Lengkap — argumen dari §5.1/§5.3 dan angka §5.2.1–§5.2.5 sudah terisi |
| BAB VI Kesimpulan dan Saran | Teks utuh; kesimpulan #1, #2, dan #3 final dengan angka nyata |
